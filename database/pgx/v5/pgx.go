...
func WithInstance(ctx context.Context, instance *pgxpool.Pool, config *Config) (database.Driver, error) {
	if config == nil {
	...
	if config.DatabaseName == "" {
		...
		if err := instance.QueryRow(ctx, query).Scan(&databaseName); err != nil {
	...
	if config.SchemaName == "" {
		...
		if err := instance.QueryRow(ctx, query).Scan(&schemaName); err != nil {
			return nil, &database.Error{OrigErr: err, Query: []byte(query)}
		}
...
func (p *Postgres) Open(ctx context.Context, url string) (database.Driver, error) {
	...
	pgconfig, err := pgxpool.ParseConfig(migrate.FilterCustomQuery(purl).String())
	if err != nil {
		return nil, err
	}
	...
	px, err := WithInstance(ctx, conn, &Config{
...
func (p *Postgres) Lock(ctx context.Context) error {
	return database.CasRestoreOnErr(&p.isLocked, false, true, database.ErrLocked, func() error {
		...
		if _, err := p.conn.Exec(ctx, query, aid); err != nil {
...
func (p *Postgres) SetVersion(ctx context.Context, version int, dirty bool) error {
	...
	if version >= 0 || (version == database.NilVersion && dirty) {
		...
		if _, err := tx.Exec(ctx, query, version, dirty); err != nil {
			if errRollback := tx.Rollback(ctx); errRollback != nil {
				err = multierror.Append(err, errRollback)
			}
			return &database.Error{OrigErr: err, Query: []byte(query)}
...
func (p *Postgres) Version(ctx context.Context) (version int, dirty bool, err error) {
	...
	err = p.conn.QueryRow(ctx, query).Scan(&version, &dirty)
	switch {
...
func (p *Postgres) Drop(ctx context.Context) (err error) {
	...
	tables, err := p.conn.Query(ctx, query)
	if err != nil {
		return &database.Error{OrigErr: err, Query: []byte(query)}
	}
	defer func() {
...
func (p *Postgres) ensureVersionTable(ctx context.Context) (err error) {
	...
	row := p.conn.QueryRow(ctx, query, p.config.migrationsSchemaName, p.config.migrationsTableName)
